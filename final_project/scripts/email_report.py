...
def send_email():
    email_receiver = get_user_setting("user_email")
    
    if not email_receiver:
        print("❌ Aucun email utilisateur configuré.")
        return

    if not EMAIL_SENDER or not EMAIL_PASSWORD:
        print("❌ Identifiants expéditeur manquants.")
        return
    ...
    df, total_val, total_pnl = get_market_data(positions)
    html_content = generate_html_body(df, total_val, total_pnl)
    
    msg = MIMEMultipart()
    msg['From'] = EMAIL_SENDER
    msg['To'] = email_receiver
    msg['Subject'] = f"Rapport Quotidien : {total_pnl:+.2f} $"
