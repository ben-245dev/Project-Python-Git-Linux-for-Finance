...
st.set_page_config(page_title="Quant Dashboard Pro", layout="wide", initial_sidebar_state="expanded")
...
def kpi_block(title, main, sub):
    st.markdown(f"""<div style="padding:8px;border-radius:7px;background:#f1f3f6;margin-bottom:8px;">
        <b style="font-size:15px">{title} :</b> <span style="font-size:22px">{main}</span>
        <br><span style="font-size:13px;color:#888">{sub}</span>
...
# Logo facultatif (à placer en local /assets/ logo_dashboard.png)
# st.sidebar.image("assets/logo_dashboard.png", width=160)
...
# --- ANALYSE GRAPHIQUE ---
if page.startswith("📈"):
    st.header("Analyse Chartist & Technique")
    ...
    if not all([close_col, open_col, high_col, low_col]) or df[close_col].dropna().size < 2:
        ...
        fig.add_trace(go.Candlestick(
            ...
            open=df[open_col], high=df[high_col], low=df[low_col], close=df[close_col], name='Bougies'))
        for tech in menu_techs:
            if tech in df:
                if "Boll" in tech:
                    line_style = dict(color="orange" if "Upper" in tech else "purple", dash="dot")
                else:
                    line_style = dict(width=2)
                fig.add_trace(go.Scatter(x=df['Date'], y=df[tech], name=tech, line=line_style))
        st.plotly_chart(fig, use_container_width=True)
        perf, dd = summarize_perf(df[close_col].dropna()/df[close_col].dropna().iloc[0])
        kpi_block("Performance", f"{perf} %", f"période {start_date.strftime('%Y-%m-%d')} → {end_date.strftime('%Y-%m-%d')}")
        kpi_block("Drawdown", f"{dd} %", "")
        st.caption("Téléchargement CSV :")
        st.download_button("Télécharger", df.to_csv(index=False), f"{ticker}_data_{start_date}_{end_date}.csv")
        st.write(df.tail(20))
    if desc_text.strip(): st.info("Commentaire : "+desc_text)

# --- BACKTEST MULTI-STRATEGIES ---
if page.startswith("🔍"):
    st.header("Backtest multi-stratégies couplées")
    with st.sidebar:
        st.markdown("**Sélectionne les stratégies et leurs paramètres**")
        ...
        combine_mode = st.selectbox("Combiner les signaux", ["Intersection (ET)", "Union (OU)"], 0)
    if st.button("Lancer le backtest multi-stratégies"):
        df = fetch_data(ticker, start_date, end_date)
        close_col = detect_col(df, ticker, "Close")
        ...
        if signal_cols:
            ...
            for i in range(1, len(df)):
                if df['signal_all'].iloc[i]:
                    equity_curve[i] = equity_curve[i-1] * (1 + returns.iloc[i])
                else:
                    equity_curve[i] = equity_curve[i-1]
            perf, dd = summarize_perf(equity_curve)
            kpi_block("Performance combinée", f"{perf} %", f"DD : {dd} %")
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=df['Date'], y=equity_curve, name="Equity", line=dict(color="blue")))
            st.plotly_chart(fig, use_container_width=True)
            fig2 = go.Figure()
            fig2.add_trace(go.Scatter(x=df['Date'], y=df[close_col], name="Prix", line=dict(color='black')))
...
st.markdown("---\nInterface professionnelle : multi-stratégies, graphiques, KPIs, screener, portefeuille et risque.")
