...
def page_strategy():
    st.title("🧪 Laboratory of Strategies and Forecast")
    ...
    if selected_strat in ["MA Crossover", "Adaptive Trend (Long/Short)", "Bollinger Breakout"]:
        params["fast_ma"] = st.sidebar.number_input("EMA Fast (Short-term Trend)", 5, 100, 20)
        params["slow_ma"] = st.sidebar.number_input("EMA Slow (Long-term Trend)", 10, 300, 50)

    if selected_strat == "RSI Reversion":
        params["rsi_len"] = st.sidebar.number_input("PPeriod RSI", 5, 30, 14)
        col_rsi1, col_rsi2 = st.sidebar.columns(2)
        params["rsi_buy"] = col_rsi1.number_input("Treshold Buy (<)", 10, 45, 30)
        params["rsi_sell"] = col_rsi2.number_input("Treshold Sell (>)", 55, 90, 70)
        
    if selected_strat == "MACD Trend":
        params["macd_fast"] = st.sidebar.number_input("MACD Fast", 5, 50, 12)
        params["macd_slow"] = st.sidebar.number_input("MACD Slow", 10, 100, 26)
        params["macd_sig"] = st.sidebar.number_input("MACD Signal", 5, 50, 9)
        
    if selected_strat == "Bollinger Breakout":
        params["bb_len"] = st.sidebar.number_input("Period Bollinger", 10, 50, 20)
        params["bb_std"] = st.sidebar.number_input("Std Dev", 1.0, 4.0, 2.0, 0.1)
        
    if selected_strat == "Adaptive Trend (Long/Short)":
        params["target_vol"] = st.sidebar.slider("Target Volatility (%)", 0.5, 5.0, 2.0, 0.1) / 100
